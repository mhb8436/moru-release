...
시연용 데이터(가상 기관 「모루시」, 문서 7건)를 로컬 환경에서 촬영한 것으로, 편집하지 않은 실제 동작이다.

<table>
  <tr>
    <td width="50%"><img src="gifs/01-search.gif" alt="통합검색"><br><b>통합검색</b> — 게시물과 기관 문서 본문을 한 번에 검색할 수 있으며, 검색어는 노란색으로 강조해 표시한다.</td>
    <td width="50%"><img src="gifs/02-doc-ai.gif" alt="문서 AI"><br><b>문서 AI</b>: LLM 키가 없는 추출 모드이다. 각 답변 문장에 「근거 확인」과 출처 번호가 표시된다.</td>
  </tr>
  <tr>
    <td><img src="gifs/04-content.gif" alt="콘텐츠 승인·게시"><br><b>콘텐츠 승인 → 게시</b> — 초안을 승인 요청한 뒤 게시하면 누리집에 바로 반영된다. 저장할 때마다 버전이 남는다.</td>
    <td><img src="gifs/03-committee.gif" alt="위원회 의결서"><br><b>위원회 의결서</b> — 회의 결과와 표수, 그리고 의결서. 의결서는 해시로 봉인해 「위변조 확인: 이상 없음」을 표시한다.</td>
  </tr>
  <tr>
    <td><img src="gifs/06-security.gif" alt="보안 점검"><br><b>보안 점검</b> — 설정을 읽어 판단하며, CSRF·비밀번호 정책·헤더·오류 노출·CORS는 자기 자신에게 요청을 보내 측정한다.</td>
    <td><img src="gifs/05-language.gif" alt="언어 전환"><br><b>한국어 ↔ English</b> — 메뉴·게시판 이름·사이트 문구는 번역 표로 관리하고, 콘텐츠는 언어별 행으로 관리한다. 해당 언어가 없으면 기본 언어로 대신 표시한다.</td>
  </tr>
</table>

## 무엇이 들어 있나

표준프레임워크는 프레임워크일 뿐 제품이 아니다. 그래서 각 사업마다 계정, 권한, 게시판, 첨부, 표준사전, 배치를 처음부터 다시 개발해야 한다. MORU는 그 빈자리를 미리 채워 둔다. 「모루」라는 이름은 대장간에서 달군 쇠를 올려놓고 두드리는 받침쇠에서 따왔다.
...
1. **HWP를 읽는다.** HWP 5.0(OLE), HWPX(ZIP+XML), HWPML(XML) 세 가지 형식을 외부 HWP 라이브러리 없이 읽을 수 있다. 확장자보다 파일 내용(매직바이트)을 먼저 확인한다. 공공 문서 중에는 확장자가 `.hwp`이지만 실제로는 HWPML인 파일이나, `.hwpx`이지만 OLE 형식인 파일도 실제로 존재한다.
2. **답변 문장마다 근거를 붙인다.** 검색된 문서 조각과 정렬해 「근거 확인 / 부분 확인 / 근거 없음」 으로 표시한다. 숫자·부정어·기관명이 출처와 다르면 「부분 확인」 으로 내린다.
3. **LLM을 끌 수 있다.** `auto` · `extract` · `rule` 세 모드는 같은 인터페이스를 사용한다. LLM 을 금지하는 사업에서는 모드를 `extract` 나 `rule` 로 내린다. 별도의 제품을 만들지 않는다.
...
- **형태** — 기관 서버 설치형이다. 세 개의 프로세스를 한 대의 서버에 함께 배치하거나 세 대의 서버로 나누어 구성한다. 클라우드 계정이나 외부 SaaS에 연결하지 않는다.
- **망분리** — AI 사이드카를 빼고 core + portal 만 두거나, LLM 호출만 DMZ squid 로 중계한다.
- **시작** — 빈 PostgreSQL 하나와 환경변수 여섯 개만 있으면 된다. Flyway가 첫 기동 시 표 36개를 생성한다. 절차는 [제품 소개서](docs/MORU-제품소개서-v1.1.pdf) 9쪽에 있다.
...
<p align="center">
  ...
  <sub>이 저장소에 포함된 이미지와 문서는 주식회사 크래프틱시스템즈의 저작물로, 제품 소개 목적으로 공개한다. 소스 코드는 별도의 저장소에 있다.</sub>
</p>
