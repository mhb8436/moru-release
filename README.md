...
시연용 데이터(가상 기관 ‘모루시’, 문서 7건)를 로컬 환경에서 촬영한 화면으로, 편집 없이 실제 동작을 그대로 담았습니다.
...
1. **HWP를 그대로 읽습니다.** HWP 5.0(OLE), HWPX(ZIP+XML), HWPML(XML) 세 가지 형식을 외부 HWP 라이브러리 없이 읽습니다. 확장자보다 파일 내용(매직바이트)을 먼저 확인합니다. 공공 문서 중에는 확장자가 `.hwp`이지만 실제로는 HWPML인 파일, `.hwpx`이지만 OLE 형식인 파일도 실제로 존재합니다.
2. **답변 문장마다 근거를 붙입니다.** 검색된 문서 조각과 대조해 「근거 확인 / 부분 확인 / 근거 없음」으로 표시합니다. 숫자·부정어·기관명이 출처와 다르면 「부분 확인」으로 판정합니다.
3. **LLM을 끌 수 있습니다.** `auto` · `extract` · `rule` 세 가지 모드가 동일한 인터페이스를 사용합니다. LLM 사용이 금지된 사업에서는 모드를 `extract` 또는 `rule`로 고정하면 되며, 별도 제품을 따로 만들지 않습니다.
