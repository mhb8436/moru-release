...
<p align="center">
  ...
  환경변수를 하나도 손대지 않은 상태에서 그대로 실행되며, AI가 필요하지 않은 사업에서는 끄고 쓰면 됩니다.
</p>
...
▶ **시연 영상(1분 59초, mp4)** — 아래 일곱 장면이 차례로 이어집니다. 장면마다 제목을 먼저 띄우고 아래쪽에 자막을 넣었습니다. [내려받기](https://github.com/mhb8436/moru-release/releases/download/v1.1/MORU-demo-v1.1.mp4) · [제품 페이지에서 바로 보기](https://mhb8436.github.io/moru-release/#demo)

가상 기관 ‘국립문서정보연구원’의 시연 데이터(문서 9건)를 넣고 로컬에서 촬영했습니다. 손댄 곳 없이 실제로 동작하는 화면 그대로입니다.

<table>
  <tr>
    <td width="50%"><img src="gifs/01-search.gif" alt="통합검색"><br><b>통합검색</b> — 게시물과 기관 문서 본문을 한 번에 찾고, 찾은 말은 노란색으로 짚어 줍니다.</td>
    <td width="50%"><img src="gifs/02-doc-ai.gif" alt="문서 AI"><br><b>문서 AI</b> — LLM 키를 넣지 않은 추출 모드로, 답변 문장마다 「근거 확인」 표시와 출처 번호가 붙습니다.</td>
  </tr>
  <tr>
    <td><img src="gifs/04-content.gif" alt="콘텐츠 승인·게시"><br><b>콘텐츠 승인 → 게시</b> — 초안을 올려 승인을 받고 게시하면 누리집에 바로 반영되며, 저장할 때마다 버전이 남습니다.</td>
  ...
  <tr>
    <td><img src="gifs/06-security.gif" alt="보안 점검"><br><b>보안 점검</b> — 설정값을 읽어 판정하고, CSRF·비밀번호 정책·보안 헤더·오류 노출·CORS 다섯 항목은 서버가 자기 자신에게 실제 요청을 보내 측정합니다.</td>
    <td><img src="gifs/05-language.gif" alt="언어 전환"><br><b>한국어 ↔ English</b> — 메뉴와 게시판 이름, 사이트 문구는 번역 표에서 관리하고, 콘텐츠는 언어마다 따로 씁니다. 해당 언어로 쓴 글이 없으면 기본 언어로 보여 줍니다.</td>
  </tr>
</table>

## 제품 구성

표준프레임워크는 말 그대로 프레임워크이며, 그대로 쓸 수 있는 제품이 아닙니다. 그래서 사업마다 계정과 권한, 게시판, 첨부, 표준사전, 배치를 처음부터 다시 만들어 왔습니다. MORU는 그 일을 미리 해 둔 제품입니다. 이름은 대장간에서 달군 쇠를 올려놓고 두드리는 받침쇠 ‘모루’에서 따왔습니다.
...
누리집과 관리 화면을 모두 KRDS 규격에 맞춘 React 컴포넌트 45종으로 조립합니다. 화면마다 마크업을 새로 짜지 않으므로, 기관이 화면을 늘려도 접근성과 디자인 규격을 따로 챙기지 않아도 됩니다. portal 안에서 이 컴포넌트를 가져다 쓰는 자리가 82곳입니다.
...
- **킷에 없는 13종** — 드로어 · 토스트 · 콤보박스 · 컨텍스트 메뉴 · 정렬되는 데이터 표. 관리 화면에는 있어야 하지만 KRDS 킷에는 없어서 직접 그렸습니다. KRDS 토큰만으로 만들어서 나머지와 섞어 놓아도 표가 나지 않습니다.
- **뼈대가 같은 관리 화면 11개** — 목록에서 「등록」을 누를 때 입력칸이 10개를 넘으면 화면 자체가 바뀌고, 그보다 적으면 모달이 열립니다. 화면마다 다르게 만들지 않았으니 담당자가 탭을 옮겨 다녀도 쓰는 방법이 같습니다.
- **서버에서 그리는 31종** — 목록이나 본문처럼 첫 화면에 바로 보여야 하는 것은 브라우저가 자바스크립트를 받을 때까지 기다리지 않습니다.
- **제품 안의 컴포넌트 문서** — `/ui`에 컴포넌트마다 한 줄 설명과 예제, 코드, 「이럴 때 쓴다 · 이럴 때 쓰지 않는다」, 변형, 속성 표를 담아 두었습니다. 왼쪽 목록도 이 라이브러리의 컴포넌트로 만들었습니다.
...
- **KRDS 킷** — 포함하지 않았습니다. krds.min.css와 글꼴은 공식 배포판 `krds-uiux`를 그대로 받아 씁니다. 망분리 환경이라면 `resources/`를 정적 폴더에 복사해 두고 씁니다.
- **프레임워크** — 가리지 않습니다. 닿는 자리가 `lib/link.tsx` 한 곳뿐이라, Next.js든 React Router든 맨 `<a>`든 그 파일만 바꾸면 동작합니다.
- **Tailwind** — 쓰신다면 `styles/krds-tailwind.css`를 같이 불러오십시오. `text-fg-subtle` 같은 이름을 KRDS 변수에 이어 줍니다. 불러오지 않아도 컴포넌트는 그대로 동작합니다.
- **규칙** — 컴포넌트마다 파일 머리에 무엇을 지켜야 하고 무엇을 하면 안 되는지 적어 두었습니다.
...
환경변수를 하나도 손대지 않아도 `./deploy/run.sh` 한 번으로 프로세스 세 개가 뜨며, 이때는 H2 파일과 SQLite+BM25, Kiwi를 씁니다. 도커도, 별도로 구매해야 하는 소프트웨어도 없습니다.
...
- **도입 형태** — 기관 서버에 설치해 씁니다. 프로세스 세 개를 한 대에 모아도 되고, 세 대로 나누어도 됩니다. AWS·Azure 같은 클라우드 계정은 필요 없고, 제품이 동작하는 데 외부 서비스를 호출하지 않습니다. 외부로 나가는 연결은 기관이 켠 것뿐입니다 — LLM, 문자(SENS), 본인확인, 동영상 게시판의 유튜브 썸네일.
- **망분리 환경** — AI 사이드카를 제외하고 core와 portal만 운영해도 되고, LLM 호출만 DMZ의 squid로 내보내도 됩니다.
- **시작하기** — 비어 있는 PostgreSQL 하나와 환경변수 여섯 개면 됩니다. 처음 실행할 때 Flyway가 테이블 36개를 만듭니다. 자세한 절차는 [제품 소개서](docs/MORU-제품소개서-v1.1.pdf) 10쪽에 있습니다.
