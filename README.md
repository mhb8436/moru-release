...
- **KRDS에 없는 13종은 직접 그렸습니다.** 드로어 · 토스트 · 콤보박스 · 컨텍스트 메뉴 · 정렬되는 데이터 표처럼 관리 화면에는 있어야 하지만 킷에는 없는 컴포넌트입니다. KRDS 토큰만으로 만들었으므로 나머지와 섞어 놓아도 표가 나지 않습니다.
- **관리 화면 11개는 뼈대가 같습니다.** 목록에서 「등록」을 누를 때 입력칸이 10개를 넘으면 화면 자체가 바뀌고, 그보다 적으면 모달이 열립니다. 화면마다 다르게 만들지 않았으므로 담당자가 탭을 옮겨 다녀도 쓰는 방법이 같습니다.
- **45종 가운데 31종은 서버에서 그립니다.** 목록이나 본문처럼 첫 화면에 바로 보여야 하는 것은 브라우저가 자바스크립트를 받을 때까지 기다리지 않습니다.
- **컴포넌트 문서 화면이 제품 안에 있습니다.** `/ui`에 컴포넌트마다 한 줄 설명과 예제, 코드, 「이럴 때 쓴다 · 이럴 때 쓰지 않는다」, 변형, 속성 표를 담아 두었습니다. 왼쪽 목록도 이 라이브러리의 컴포넌트로 만들었습니다.
...
- **KRDS 킷은 포함하지 않았습니다.** krds.min.css와 글꼴은 공식 배포판 `krds-uiux`를 그대로 받아 씁니다. 망분리 환경이라면 `resources/`를 정적 폴더에 복사해 두고 씁니다.
- **프레임워크를 가리지 않습니다.** 프레임워크에 닿는 자리가 `lib/link.tsx` 한 곳뿐이라, Next.js든 React Router든 맨 `<a>`든 그 파일만 바꾸면 동작합니다.
- **Tailwind를 쓰신다면** `styles/krds-tailwind.css`를 같이 불러오십시오. `text-fg-subtle` 같은 이름을 KRDS 변수에 이어 줍니다. 불러오지 않아도 컴포넌트는 그대로 동작합니다.
- **규칙은 소스에 적어 두었습니다.** 컴포넌트마다 파일 머리에 무엇을 지켜야 하고 무엇을 하면 안 되는지 적어 두었습니다.
